...
<div style="background:#eee; padding: 20px 5px">
The idea to publish a service like Cronboy appeared when we have build web service that schedule jobs for our internal project. We notice that handling such kind of jobs is a general pattern and is boring to write those code from scratch for every project which need to execute  this type of work. We need some kind of feedback. Is it interesting for laravel web developers? Please leave some feadback, ideas, or questions to the github issues if you can.
</div>
